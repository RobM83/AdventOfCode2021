...
func increaseGrid(grid [][]int, multiply int) [][]int {
	newGrid := make([][]int, len(grid)*multiply)
	for y := 0; y < len(newGrid); y++ {
		newGrid[y] = make([]int, len(grid[0])*multiply)
	}
	...
	for y := 0; y < len(newGrid); y++ {
		for x := 0; x < len(newGrid[y]); x++ {
			...
			if grid[orgY][orgX]+inc >= 10 {
				newValue = grid[orgY][orgX] + inc - 9
			} else {
				newValue = grid[orgY][orgX] + inc
			}
